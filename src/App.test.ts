...
const mocks = vi.hoisted(() => ({
  ...
  getScanSettings: vi.fn(), listProviderDescriptors: vi.fn(),
}));
...
describe('App continuation boundary', () => {
  ...
  beforeEach(() => {
    ...
    mocks.listProviderDescriptors.mockResolvedValue([{
  ...
  it('shows scan errors after exit instead of reporting a missing child', async () => {
    ...
    releasePoll({ handle: 'fork-h', session_id: 'child', parent_session_id: 'parent', status: 'exited', events: [], live_events: [], tail_partial: false, tail_diagnostics: 0, tail_error: null, tail_caught_up: true });
    await settle();
    expect(root.textContent).toContain('Scan/sync incomplete: scan unavailable');
    expect(root.textContent).not.toContain('no new source');
  });
  ...
  it('reports a missing child only after a successful scan confirms absence', async () => {
    ...
    releasePoll({ handle: 'fork-h', session_id: 'child', parent_session_id: 'parent', status: 'exited', events: [], live_events: [], tail_partial: false, tail_diagnostics: 0, tail_error: null, tail_caught_up: true });
    await settle();
    expect(root.textContent).toContain('no new source');
  });
  ...
  it('shows an explicit automatic scan settings error', async () => {
    ...
    app.mount(root);
    await settle();
    expect(root.textContent).toContain('Automatic scan settings unavailable: settings unavailable');
  });

  it('shows the cached index before the startup scan finishes', async () => {
    await settle();
    app.unmount();
    let finishScan!: (value: ScanReport) => void;
    mocks.scan.mockReset().mockReturnValueOnce(new Promise((resolve) => { finishScan = resolve; }));
    app = createApp(App);
    app.mount(root);
    ...
    expect(root.querySelector<HTMLButtonElement>('.refresh-button')?.textContent).toContain('Scanning');
