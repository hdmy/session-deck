/** Provider identifiers are namespaced anywhere they are used as keys. */
export type ProviderId = 'claude' | 'codex' | 'gemini' | 'opencode' | (string & {});

/** Capabilities are serialized by Rust with snake_case field names. */
...
export type KnowledgeRelationType = 'semantic' | 'duplicate' | 'solution' | 'reference';
...
export interface KnowledgeCardPatch {
  title?: string;
  summary?: string;
  tags?: string[];
  body_markdown?: string;
...
export interface Project {
  id: string;
  workspace_id?: string;
  /** Providers contributing sessions to this workspace. */
  ...
  /** Provider-specific session groups; sessions remains the flat compatibility view. */
  agents?: AgentSummary[];
}
