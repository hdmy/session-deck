...
const phaseLabel = computed(() => ({ idle: 'Ready', preflighting: 'Checking…', starting: 'Starting', running: 'Running', draining: 'Draining transcript…', exited: 'Exited', error: 'Error', closed: 'Closed' })[phase.value]);
...
<template>
  <section class="continuation-dock" :style="{ height: minimized ? '48px' : `${height}px` }" aria-label="Claude continuation terminal">
    ...
    <header class="terminal-head"><div><strong>{{ props.mode === 'fork' ? 'Claude fork terminal' : 'Claude terminal' }}</strong><span>{{ title }} · Runtime output is written by Claude CLI, then synced after exit.</span><span v-if="props.mode === 'fork'" class="fork-notice" role="status">PTY 实时；子 source 待建立 baseline，退出后完整同步</span><span class="terminal-status" role="status" aria-live="polite">{{ phaseLabel }}</span></div><div class="terminal-controls"><button type="button" @click="minimized = !minimized">{{ minimized ? 'Expand' : 'Minimize' }}</button><button type="button" @click="closeTerminal">Close</button></div></header>
    <div v-if="!minimized" ref="terminal-host" class="terminal-host"></div>
    <div v-if="phase === 'starting' && !minimized" class="terminal-overlay">Starting Claude…</div>
    <div v-if="terminalError && !minimized" class="terminal-error" role="alert">{{ terminalError }}</div>
    <div v-if="tailError && !minimized" class="terminal-error tail-error" role="alert">Live transcript: {{ tailError }}</div>
    <div v-if="!minimized && (tailPartial || tailDiagnostics)" class="terminal-tail-notice" role="status">
      Live transcript{{ tailPartial ? ' is partial' : '' }}<span v-if="tailDiagnostics"> · {{ tailDiagnostics }} diagnostics</span>
    </div>
  </section>
</template>
