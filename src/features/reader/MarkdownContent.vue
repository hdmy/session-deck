...
<template>
  <section class="markdown-content">
    <header class="markdown-toolbar">
      <div class="markdown-mode" role="group" :aria-label="t('responseFormat')">
        <button type="button" :class="{ active: mode === 'rendered' }" :aria-pressed="mode === 'rendered'" @click="mode = 'rendered'">{{ t('rendered') }}</button>
        <button type="button" :class="{ active: mode === 'raw' }" :aria-pressed="mode === 'raw'" @click="mode = 'raw'">{{ t('raw') }}</button>
      </div>
      <button type="button" class="copy-markdown" :aria-label="t('responseCopy')" :aria-describedby="copyStatus === 'error' ? 'copy-markdown-error' : undefined" @click="copy">{{ copyStatus === 'copied' ? t('copied') : copyStatus === 'error' ? t('copyFailed') : t('copy') }}</button>
      <span v-if="copyStatus === 'error'" id="copy-markdown-error" class="copy-error" role="alert">{{ t('clipboardUnavailable') }}</span>
    </header>
...
<style scoped>
.markdown-content { border-left: none; padding-left: 0; }
