...
describe('LiveTranscriptPanel', () => {
  ...
  it('shows no-new-event, partial, diagnostics, and tail error states', async () => {
    ...
    await nextTick();
    expect(root.textContent).toContain('正在收尾');
    expect(root.textContent).toContain('正在收尾 transcript');
    expect(root.textContent).toContain('尚未完整落盘');
    expect(root.textContent).toContain('2 条诊断');
