...
describe('useSessionBrowser', () => {
  ...
  beforeEach(() => {
    ...
    apiMock.activateClaudeSourceRoot.mockResolvedValue({ settings: { source_root: '/tmp', effective_root: '/tmp', scan_interval_seconds: 0 }, scan: makeReport() });
  ...
  it('keeps sorting state fresh after pin and touch mutations', async () => {
    ...
    apiMock.touchSession.mockResolvedValue({ ...recent, last_used_at: 99 });
    apiMock.projects.mockResolvedValue([makeProject([{ ...old, pinned: true }, { ...recent, last_used_at: 99 }])]);
  ...
  it('serializes root activation after a pre-existing scan so the activation report wins', async () => {
    ...
    apiMock.activateClaudeSourceRoot.mockResolvedValueOnce({
      settings: { source_root: '/new-root', effective_root: '/new-root', scan_interval_seconds: 0 },
  ...
  it('keeps later scans behind an in-flight root activation', async () => {
    await browser.refresh();
    let releaseActivation!: (value: { settings: { source_root: string; effective_root: string; scan_interval_seconds: number }; scan: ScanReport }) => void;
    ...
    expect(browser.scanNotice.value).toContain('新增 1');
    expect(browser.scanning.value).toBe(false);
  });
  ...
  it('lets a scheduled commit created after activation return win', async () => {
    ...
    releaseActivation({ settings: { source_root: '/new-root', effective_root: '/new-root', scan_interval_seconds: 0 }, scan: makeReport({ root: '/new-root', partial: true, partial_sessions: 1 }) });
    await activation;
    expect((await scheduled).status).toBe('success');
  ...
  it('reserves an idle activation lease before a near-synchronous refresh', async () => {
    ...
    expect(apiMock.scan).toHaveBeenCalledTimes(1);
    releaseActivation({ settings: { source_root: '/new-root', effective_root: '/new-root', scan_interval_seconds: 0 }, scan: makeReport({ root: '/new-root' }) });
    await activation;
    expect(browser.scanning.value).toBe(false);
  });
  ...
  it('shows a content-free notice when source sessions are removed', async () => {
    ...
    expect(browser.sourceRemovalNotice.value).toContain('移除 3 个 Session');
    expect(browser.sourceRemovalNotice.value).toContain('不同于“隐藏”');
  ...
  it('returns a visible skipped result without hydrating an index', async () => {
    ...
    expect(browser.scanNotice.value).toContain('跳过');
