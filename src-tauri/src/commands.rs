use crate::{
    domain::{
        AppError, AppStatus, Project, Result, ScanReport, SearchHit, SessionDetail, SessionSummary,
    },
    indexer,
    providers::{
...
impl AppState {
    pub fn new() -> Result<Self> {
        ...
        let db = storage::open(&data.data_dir().join("index.db"))?;
        let configured_root = storage::claude_scan_settings(&db)?
            .source_root
            .map(PathBuf::from)
...
#[tauri::command]
pub fn scan(state: State<'_, AppState>, args: Option<ScanArgs>) -> Result<ScanReport> {
    ...
    let old_manifest = {
        ...
        storage::source_manifest(&db, PROVIDER_ID)?
    };
    let result = prepare_scan(&root, &old_manifest);
    ...
    let report = match result {
        Ok((
            ...
            attempt_codes = errors.clone();
            let count = discovered_paths.len();
            ...
            let partial_sessions = sessions
                ...
                .count();
            let diagnostic_count = errors.len().max(parser_diagnostics);
            if complete {
                ...
                match indexer::reconcile_incremental(
                    ...
                    Ok(removed_sessions) => {
                        ...
                        *state.last_scan.lock().expect("time lock") =
                        ...
                        Ok(ScanReport {
                            ...
                            partial: diagnostic_count > 0,
                            removed_sessions,
                            new_files,
                            changed_files,
                            unchanged_files,
                            removed_files: old_manifest
                                .keys()
    ...
    if let Ok(ref value) = report {
        let db = state
            .db
            .lock()
            .map_err(|_| AppError::Message("database lock poisoned".into()))?;
        let counts = diagnostic_counts(&attempt_codes);
        storage::record_scan_run(
            &db,
            scan_started_at,
            &value.trigger,
...
pub fn global_search(state: State<'_, AppState>, query: String) -> Result<Vec<SearchHit>> {
    search::query(&state.db.lock().expect("db lock"), &query)
}
...
pub fn update_scan_settings(
    ...
    storage::update_claude_scan_settings(&db, &settings)?;
    Ok(ScanSettingsDto {
        source_root: settings.source_root.clone(),
        effective_root: settings
            .source_root
