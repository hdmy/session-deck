use context_vault_lib::{
    domain::KnowledgeCardPatch, knowledge, providers::claude::parse_session, scanner, search,
    storage,
};
use std::io::Write;
...
#[test]
fn knowledge_cards_auto_manual_backfill_sources_and_semantic_search() {
    let source = tempdir().unwrap();
    let project = source.path().join("knowledge-project");
    std::fs::create_dir_all(&project).unwrap();
    let first_path = project.join("11111111-1111-4111-8111-111111111111.jsonl");
    let second_path = project.join("22222222-2222-4222-8222-222222222222.jsonl");
    write_session(
        &first_path,
        "{\"type\":\"user\",\"sessionId\":\"11111111-1111-4111-8111-111111111111\",\"message\":{\"role\":\"user\",\"content\":\"修复 SQLite migration error\"}}\n{\"type\":\"assistant\",\"sessionId\":\"11111111-1111-4111-8111-111111111111\",\"message\":{\"role\":\"assistant\",\"content\":\"决定采用 migration，问题已修复\"}}\n",
    );
    write_session(
        &second_path,
        "{\"type\":\"user\",\"sessionId\":\"22222222-2222-4222-8222-222222222222\",\"message\":{\"role\":\"user\",\"content\":\"SQLite migration tests\"}}\n{\"type\":\"assistant\",\"sessionId\":\"22222222-2222-4222-8222-222222222222\",\"message\":{\"role\":\"assistant\",\"content\":\"测试通过\"}}\n",
    );
    let first = parse_session(&first_path).unwrap();
    let second = parse_session(&second_path).unwrap();
    let dbdir = tempdir().unwrap();
    let mut connection = storage::open(&dbdir.path().join("index.db")).unwrap();
    storage::index(&mut connection, "claude", &[first.clone(), second.clone()]).unwrap();

    let initial = storage::get_knowledge_card(&connection, &first.summary.id).unwrap();
    assert!(initial.tags.contains(&"数据库".to_string()));
    storage::update_knowledge_card(
        &mut connection,
        &first.summary.id,
        &KnowledgeCardPatch {
            tags: Some(vec!["manual".into()]),
            body_markdown: Some("manual body".into()),
            source_session_ids: Some(vec![second.summary.id.clone()]),
            ..Default::default()
        },
    )
    .unwrap();
    let edited = storage::get_knowledge_card(&connection, &first.summary.id).unwrap();
    assert_eq!(edited.tags, vec!["manual"]);
    assert_eq!(edited.body_markdown, "manual body");
    assert!(edited.source_session_ids.contains(&first.summary.id));
    assert!(edited.source_session_ids.contains(&second.summary.id));
    storage::index(&mut connection, "claude", &[first.clone(), second.clone()]).unwrap();
    let after_reindex = storage::get_knowledge_card(&connection, &first.summary.id).unwrap();
    assert_eq!(after_reindex.tags, vec!["manual"]);
    assert_eq!(after_reindex.body_markdown, "manual body");

    let related =
        storage::related_sessions(&connection, &first.summary.id, Some("claude"), 10).unwrap();
    assert!(related
        .iter()
        .any(|item| item.session.id == second.summary.id));
    let semantic =
        storage::semantic_search(&connection, "SQLite migration", Some("claude"), 10).unwrap();
    assert!(!semantic.is_empty());
    assert!(!semantic[0].summary.is_empty());
    assert!(semantic[0].topics.len() + semantic[0].tags.len() > 0);

    connection
        .execute("DELETE FROM knowledge_cards", [])
        .unwrap();
    connection
        .execute("DELETE FROM knowledge_card_sources", [])
        .unwrap();
    assert_eq!(storage::backfill_missing_knowledge(&connection).unwrap(), 2);
    assert!(!storage::get_knowledge_card(&connection, &first.summary.id)
        .unwrap()
        .summary
        .is_empty());

    storage::index(&mut connection, "claude", &[first]).unwrap();
    assert!(storage::get_knowledge_card(&connection, &second.summary.id).is_err());
}

#[test]
fn reopening_an_existing_index_backfills_missing_knowledge_cards() {
    let source = tempdir().unwrap();
    let project = source.path().join("knowledge-reopen-project");
    std::fs::create_dir_all(&project).unwrap();
    let session_path = project.join("33333333-3333-4333-8333-333333333333.jsonl");
    write_session(
        &session_path,
        "{\"type\":\"user\",\"sessionId\":\"33333333-3333-4333-8333-333333333333\",\"message\":{\"role\":\"user\",\"content\":\"修复 SQLite migration error\"}}\n{\"type\":\"assistant\",\"sessionId\":\"33333333-3333-4333-8333-333333333333\",\"message\":{\"role\":\"assistant\",\"content\":\"问题已修复并完成测试\"}}\n",
    );
    let parsed = parse_session(&session_path).unwrap();
    let dbdir = tempdir().unwrap();
    let database = dbdir.path().join("index.db");
    {
        let mut connection = storage::open(&database).unwrap();
        storage::index(&mut connection, "claude", std::slice::from_ref(&parsed)).unwrap();
        connection
            .execute("DELETE FROM knowledge_card_sources", [])
            .unwrap();
        connection
    ...
    let card = storage::get_knowledge_card(&connection, &parsed.summary.id).unwrap();
...
#[test]
fn knowledge_results_skip_hidden_and_keep_explicit_low_score_reference() {
    let source = tempdir().unwrap();
    let project = source.path().join("knowledge-wire-project");
    std::fs::create_dir_all(&project).unwrap();
    let first_path = project.join("33333333-3333-4333-8333-333333333333.jsonl");
    let second_path = project.join("44444444-4444-4444-8444-444444444444.jsonl");
    write_session(
        &first_path,
        "{\"type\":\"user\",\"sessionId\":\"33333333-3333-4333-8333-333333333333\",\"message\":{\"role\":\"user\",\"content\":\"SQLite migration repair\"}}\n",
    );
    write_session(
        &second_path,
        "{\"type\":\"user\",\"sessionId\":\"44444444-4444-4444-8444-444444444444\",\"message\":{\"role\":\"user\",\"content\":\"unrelated deployment notes\"}}\n",
    );
    let first = parse_session(&first_path).unwrap();
    let second = parse_session(&second_path).unwrap();
    let dbdir = tempdir().unwrap();
    ...
    storage::set_session_hidden(&connection, &second.summary.id, true).unwrap();
    let hidden_results =
        storage::semantic_search(&connection, "deployment notes", Some("claude"), 10).unwrap();
    assert!(!hidden_results
        .iter()
        .any(|hit| hit.session.id == second.summary.id));
    storage::set_session_hidden(&connection, &second.summary.id, false).unwrap();

    storage::update_knowledge_card(
        &mut connection,
        &second.summary.id,
        &KnowledgeCardPatch {
            source_session_ids: Some(vec![first.summary.id.clone()]),
            ..Default::default()
        },
    )
    .unwrap();
    let first_blob: Vec<u8> = connection
        .query_row(
            "SELECT vector FROM knowledge_cards WHERE session_id = ?",
            [&first.summary.id],
            |row| row.get(0),
        )
        .unwrap();
    let first_vector = knowledge::decode_vector(&first_blob).unwrap();
    let opposite = first_vector
        .into_iter()
        .map(|value| -value)
        .collect::<Vec<_>>();
    let opposite_blob = knowledge::encode_vector(&opposite).unwrap();
    connection
        .execute(
            "UPDATE knowledge_cards SET vector = ? WHERE session_id = ?",
            rusqlite::params![opposite_blob, second.summary.id],
        )
        .unwrap();

    let related =
        storage::related_sessions(&connection, &first.summary.id, Some("claude"), 10).unwrap();
    let reference = related
        .iter()
        .find(|hit| hit.session.id == second.summary.id)
        .expect("explicit reference remains visible");
    assert_eq!(reference.relation_type, "reference");
    assert!(reference.score < 0.0);
    assert!(!reference.summary.is_empty());
}

#[test]
fn v10_database_without_knowledge_tables_migrates_and_reopens() {
    let root = tempdir().unwrap();
    let path = root.path().join("v10.db");
    let connection = storage::open(&path).unwrap();
    connection
        .execute_batch(
            "DROP TABLE knowledge_card_sources;
...
#[test]
fn knowledge_failure_rolls_back_session_source_and_card_together() {
    let source = tempdir().unwrap();
    let project = source.path().join("atomic");
    std::fs::create_dir_all(&project).unwrap();
    let path = project.join("atomic.jsonl");
    write_session(
        &path,
        "{\"type\":\"user\",\"sessionId\":\"atomic\",\"message\":{\"role\":\"user\",\"content\":\"atomic migration\"}}\n",
    );
    let parsed = parse_session(&path).unwrap();
    let root = tempdir().unwrap();
    let mut connection = storage::open(&root.path().join("index.db")).unwrap();
    connection
        .execute_batch(
            "CREATE TRIGGER reject_knowledge BEFORE INSERT ON knowledge_cards
             BEGIN SELECT RAISE(ABORT, 'synthetic knowledge failure'); END;",
        )
        .unwrap();
    assert!(storage::index(&mut connection, "claude", &[parsed]).is_err());
    assert_eq!(storage::count(&connection).unwrap(), 0);
    assert_eq!(
        connection
    ...
    assert_eq!(
        connection
            .query_row("SELECT COUNT(*) FROM knowledge_cards", [], |row| row
                .get::<_, i64>(0))
            .unwrap(),
        0
    );
...
#[test]
fn malformed_derived_cards_are_skipped_but_single_card_errors() {
    let source = tempdir().unwrap();
    let project = source.path().join("derived");
    std::fs::create_dir_all(&project).unwrap();
    let first_path = project.join("first.jsonl");
    let second_path = project.join("second.jsonl");
    write_session(&first_path, "{\"type\":\"user\",\"sessionId\":\"first\",\"message\":{\"role\":\"user\",\"content\":\"SQLite migration\"}}\n");
    write_session(&second_path, "{\"type\":\"user\",\"sessionId\":\"second\",\"message\":{\"role\":\"user\",\"content\":\"SQLite migration tests\"}}\n");
    let first = parse_session(&first_path).unwrap();
    let second = parse_session(&second_path).unwrap();
    let root = tempdir().unwrap();
    let mut connection = storage::open(&root.path().join("index.db")).unwrap();
    storage::index(&mut connection, "claude", &[first.clone(), second]).unwrap();
    connection
        .execute(
            "UPDATE knowledge_cards SET auto_json = 'not-json' WHERE session_id = ?",
            [&first.summary.id],
        )
        .unwrap();
    assert!(storage::get_knowledge_card(&connection, &first.summary.id).is_err());
    let results =
        storage::semantic_search(&connection, "SQLite migration", Some("claude"), 50).unwrap();
    assert!(results
        .iter()
        .all(|item| item.session.id != first.summary.id));
    assert!(!results.is_empty());
}

#[test]
fn knowledge_patch_limits_and_unicode_query_are_explicit() {
    let source = tempdir().unwrap();
    let project = source.path().join("limits");
    std::fs::create_dir_all(&project).unwrap();
    let path = project.join("limits.jsonl");
    write_session(&path, "{\"type\":\"user\",\"sessionId\":\"limits\",\"message\":{\"role\":\"user\",\"content\":\"limits\"}}\n");
    let parsed = parse_session(&path).unwrap();
    let root = tempdir().unwrap();
    let mut connection = storage::open(&root.path().join("index.db")).unwrap();
    storage::index(&mut connection, "claude", &[parsed]).unwrap();
    assert!(storage::semantic_search(&connection, &"字".repeat(4097), None, 1).is_err());
    assert!(storage::semantic_search(&connection, "limits", None, 0).is_err());
    assert!(storage::update_knowledge_card(
        &mut connection,
        "claude:limits",
        &KnowledgeCardPatch {
            tags: Some((0..33).map(|value| value.to_string()).collect()),
            ..Default::default()
        },
    )
    .is_err());
...
#[test]
fn provider_namespace_filter_and_project_agents_are_scoped() {
    let source = tempdir().expect("source");
    let project = source.path().join("project");
    std::fs::create_dir_all(&project).expect("project");
    let claude_path = project.join("claude.jsonl");
    let codex_path = project.join("codex.jsonl");
    write_session(
        &claude_path,
        "{\"type\":\"user\",\"sessionId\":\"shared\",\"message\":{\"role\":\"user\",\"content\":\"claude-only\"}}\n",
    );
    write_session(
        &codex_path,
        "{\"type\":\"user\",\"sessionId\":\"shared\",\"message\":{\"role\":\"user\",\"content\":\"codex-only\"}}\n",
    );
    let mut claude = parse_session(&claude_path).expect("claude parse");
    let mut codex = parse_session(&codex_path).expect("codex parse");
    claude.summary.workspace_id = "/repo/.git".into();
    claude.summary.project_path = Some("/repo".into());
    codex.summary.workspace_id = "codex:dir:/repo".into();
    codex.summary.project_path = Some("/repo".into());
    codex.summary.provider_id = "codex".into();
    codex.summary.id = "codex:shared".into();
    for event in &mut codex.events {
        event.session_id = "codex:shared".into();
    }
    for turn in &mut codex.turns {
        turn.session_id = "codex:shared".into();
    }
    for branch in &mut codex.branches {
        branch.summary.session_id = "codex:shared".into();
    ...
    let mut connection = storage::open(&database.path().join("index.db")).expect("open");
    storage::index(&mut connection, "claude", &[claude]).expect("index claude");
    storage::index(&mut connection, "codex", &[codex]).expect("index codex");

    assert_eq!(
        storage::search_filtered(&connection, "only", Some("codex"))
            .unwrap()
            .len(),
        1
    );
    assert_eq!(
        storage::search_filtered(&connection, "only", Some("claude"))
            .unwrap()
            .len(),
        1
    );
    ...
    assert_eq!(
        projects[0]
            .agents
            .iter()
            .find(|agent| agent.provider_id == "codex")
            .map(|agent| agent.name.as_str()),
        Some("Codex")
    );
    storage::set_project_alias(&connection, "codex", "codex:dir:/repo", Some("Codex")).unwrap();
    let project = storage::projects(&connection).unwrap().remove(0);
    assert_eq!(
        project
            .agents
            .iter()
            .find(|a| a.provider_id == "codex")
            .unwrap()
            .alias
            .as_deref(),
        Some("Codex")
...
#[test]
fn scan_settings_default_to_claude_and_prune_disabled_provider_rows() {
    let database = tempdir().expect("create database root");
    let mut connection = storage::open(&database.path().join("index.db")).expect("open database");
    connection
        .execute_batch(
            "INSERT INTO sessions
             ...
             VALUES
                ...
                ('codex:one', 'codex', 'project', 'Codex', '[]', 0, 0, 0);
             INSERT INTO search_chunks (session_id, event_id, field, content)
             VALUES ('codex:one', 0, 'title', 'Codex');",
        )
        .expect("seed provider rows");

    let settings = storage::claude_scan_settings(&connection).expect("read scan settings");
    assert_eq!(settings.enabled_provider_ids, vec!["claude"]);
    storage::update_claude_scan_settings(&mut connection, &settings).expect("save settings");
    ...
    assert_eq!(
        connection
            .query_row("SELECT COUNT(*) FROM search_chunks", [], |row| row
                .get::<_, i64>(0))
            .unwrap(),
        0
    );
...
#[test]
fn last_successful_scan_time_survives_reopen() {
    let database = tempdir().expect("create database root");
    let path = database.path().join("index.db");
    {
        let connection = storage::open(&path).expect("open database");
        storage::record_scan_run(&connection, 123, "manual", "partial", true, 1, &[])
            .expect("record successful partial scan");
    }
    let reopened = storage::open(&path).expect("reopen database");
    assert_eq!(storage::last_success_at(&reopened).unwrap(), Some(123));
}
