...
- [x] P0：用户实际启动桌面应用并完成一次人工验收。
- [x] P1：本地历史管理与体验增强已实现并完成用户验收。
- [ ] P2：多 Agent Provider 与本地知识库已实现，待用户人工验收。
...
- [x] 启动桌面应用，确认真实项目树和 Session 标题符合预期。
- [x] 搜索一个历史关键词（例如 `CloudBase`），确认能跨项目定位 Session。
- [x] 打开真实 Session，确认同一轮对话没有被错误拆成多个顶层记录。
- [x] 手动执行一次 preflight，并在确认后测试一次真实 `继续对话`。
- [x] 确认续聊结束后 Session 列表和 Reader 出现新 turn。
...
- [x] Codex session provider adapter。
- [x] Gemini CLI session provider adapter。
- [x] OpenCode session provider adapter。
- [x] provider capabilities 映射：reader、search、resume、branching、worktree。
- [x] 统一 Project → Agent → Session → Context 展示模型。
- [x] 跨 Agent 全局搜索和 provider 筛选。

### AI Coding 工作历史知识库

- [x] 基于 AI Coding transcript 的本地确定性自动 summary、topics、tags（不调用远程模型）。
- [x] 基于 256 维本地特征向量的语义检索。
- [x] 相关 Session、重复问题和解决方案关联。
- [x] 从 Session 提取可复用的架构决策、排障经验和代码变更摘要。
- [x] 可人工编辑的知识卡片与引用回链。
...
1. [x] 先实现“隐藏”与本地索引状态模型。
2. [x] 再实现 live tail 和续聊期间的状态同步。
3. [x] 再补项目/worktree 与索引增量能力。
4. [x] 最后评估 Codex、Gemini CLI、OpenCode provider 的数据样本和 parser 方案。
